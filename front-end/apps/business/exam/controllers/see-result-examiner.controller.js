                ...
                function init() {
                    ...
                    dataManupulator.getConnections(connectionFilter).then(function (res) {
                        ...
                        if(connections.length<=0){
                            ...
                            dataManupulator.manupulate("getMany", {entityName:"exam", filters:{_id:{$in:examIds}}}).then(function (res) {
                                var response = res.data.data;
                                response.forEach(function (item) {
                                    exams[item._id] = item;
                                });
                                dataManupulator.manupulate("getMany",{entityName:"user", filters:{_id:{$in:examineeIds}}}).then(function (res) {
                                    var response = res.data.data;
                                    response.forEach(function (item) {
                                        examinees[item._id] = item;
                                    });
                                    dataManupulator.manupulate("getMany", {entityName:"answer",
                                        filters:{examId:{$in:examIds}, examineeId: {$in: examineeIds}}}).then(function (res) {
                                        var response = res.data.data;
                                        response.forEach(function (item) {
                                            var result = {};
